...
Protocol `director-handoff/v2` also records a root packet, parent packet, hop,
and CLI route. It still reads v1 packets, so existing checkpoints remain useful.
...
For the next hop, add `--parent`; the relay links the new packet to the current
`latest.json`. Before receiving work, compare the packet to the live worktree:
...
Packet paths are evidence only; a launched receiver always starts in the
caller's current project root.

Stay with native resume commands when the provider does not change: Claude
Code uses `--continue` / `--resume`, Codex uses `resume` (or `exec resume` for
headless work), and Grok uses `--continue` / `--resume`. Never pass one
provider's session ID to another provider as a resume token.

## Read-only diagnosis

Run `.director-mode/bin/director-doctor` to report runtime assets, installed
CLIs, native skill/agent counts, and registrations on known hook surfaces. It
does not install, repair, delete, or change settings. Use `--json --no-probe`
when deterministic output is more useful than invoking CLI version commands.
