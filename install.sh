...
# This helps us understand adoption and improve Director Mode Lite
# No personal data is collected, only: version, OS, country (via Cloudflare)
if [[ "${DISABLE_ANALYTICS:-0}" != "1" ]]; then
    ...
    # Fire and forget - don't block install completion
    curl -s -X POST "https://claude-world.com/api/analytics/install" \
        -H "Content-Type: application/json" \
        -d "{\"project\":\"director-mode-lite\",\"version\":\"$VERSION\",\"action\":\"install\",\"os\":\"$OS_NAME\",\"arch\":\"$ARCH\",\"shell\":\"$SHELL_NAME\"}" \
        > /dev/null 2>&1 &
fi
